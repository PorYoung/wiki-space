...
1. 现有产品的普遍缺口是"源"：要么只认 Git，要么完全不接外部源。ewiki 以 4 类源（Git / 本地文件夹 / 网页链接 / 数据库，其中数据库支持 10 种）为一等公民，把"接入-同步-冲突"做成闭环，这是核心差异点。
...
| F19 | 数据源 | 添加源 | 弹窗按 4 种类型（Git 仓库 / 本地文件夹 / 网页链接 / 数据库）提供不同表单，见 6.2.4 分支图 |
