...
import { fetchProject, fetchTeam } from '../api/stubs.js'
...
export default function ProjectMembers() {
  ...
  useEffect(() => {
    ;(async () => {
      const [proj, allTeam] = await Promise.all([fetchProject(id), fetchTeam()])
      setProject(proj)
      // 过滤出该项目的成员（从项目 members 字段，fallback 到全局 team）
      const projectMembers = proj?.members && proj.members.length > 0
        ? allTeam.filter((m) => proj.members.includes(m.id))
        : allTeam.slice(0, 6)
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-neutral-500 mt-1">
              {project?.name || '当前项目'} · 邀请成员、管理角色与权限
            </p>
          </div>
